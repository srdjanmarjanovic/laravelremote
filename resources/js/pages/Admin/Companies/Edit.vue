...
import { Upload, X, Building2, Globe, Twitter, Linkedin, Github } from 'lucide-vue-next';
...
const submit = () => {
    // Use POST with _method: 'put' when files are included to avoid multipart/form-data issues with PUT
    // This ensures all form fields are properly sent with file uploads
    if (form.logo) {
        form.transform((data) => ({
            ...data,
            _method: 'put',
        })).post(admin.companies.update({ company: props.company.id }).url, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: () => {
                toast.error('There was an error updating the company. Please check the form.');
            },
        });
    } else {
        form.put(admin.companies.update({ company: props.company.id }).url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Company updated successfully!');
            },
...
const breadcrumbs = [
    ...
    {
        title: 'Edit Company',
        href: admin.companies.edit({ company: props.company.id }).url,
    },
];
</script>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>The official name of the company</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Input
                            v-model="form.name"
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Tell developers about the company, culture, and mission</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Textarea
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>The company's main website (optional)</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Add links to the company's social profiles (optional)</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-4">
                        <div class="space-y-2">
                ...
                <div class="flex items-center justify-end gap-4">
                    <Button
                        type="button"
                        variant="outline"
                        @click="router.visit(admin.companies.index().url)"
                    >
                        Cancel
                    </Button>
